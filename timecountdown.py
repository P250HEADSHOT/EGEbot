...
def handle_text(message):
    ...
    bot.send_message(message.chat.id,
                     "До Русича осталось {} дней, {} часов {} минут {} секунд.".format(drus.days, drus.seconds // 3600,
...
def handle_text(message):
    now = datetime.datetime.now()
    dmath = math - now
    bot.send_message(message.chat.id, "До Матеши осталось {} дней, {} часов {} минут {} секунд.".format(dmath.days,
                                                                                                        dmath.seconds // 3600,
...
def handle_text(message):
    now = datetime.datetime.now()
    dphys = phys - now
    bot.send_message(message.chat.id, "До Физона осталось {} дней, {} часов {} минут {} секунд.".format(dphys.days,
                                                                                                        dphys.seconds // 3600,
...
def handle_text(message):
    ...
    dit = it - now
    bot.send_message(message.chat.id,
                     "До ИКТ осталось {} дней, {} часов {} минут {} секунд.".format(dit.days, dit.seconds // 3600,
                                                                                    dit.seconds % 3600 // 60,
                                                                                    dit.seconds % 60),
                     reply_to_message_id=message.message_id)
...
def handle_text(message):
    if ("скока до русича" in str.lower(message.text)) or ("cкока до русского" in str.lower(message.text)) or (
        "cколько до русича" in str.lower(message.text)) or ("cколько до русского" in str.lower(message.text)):
        now = datetime.datetime.now()
        drus = rus - now
        bot.send_message(message.chat.id, "До Русича осталось {} дней, {} часов {} минут {} секунд.".format(drus.days,
                                                                                                            drus.seconds // 3600,
                                                                                                            drus.seconds % 3600 // 60,
                                                                                                            drus.seconds % 60)
                         , reply_to_message_id=message.message_id)
    if ("cкока до матеши" in str.lower(message.text)) or ("сколько до матеши" in str.lower(message.text)) or (
        "скока до математики" in str.lower(message.text)) or ("сколько до математики" in str.lower(message.text)):
        now = datetime.datetime.now()
        dmath = math - now
        bot.send_message(message.chat.id, "До Матеши осталось {} дней, {} часов {} минут {} секунд.".format(dmath.days,
                                                                                                            dmath.seconds // 3600,
                                                                                                            ...
                                                                                                            dmath.seconds % 60),
                         reply_to_message_id=message.message_id)
    if ("скока до физеки" in str.lower(message.text)) or ("сколько до физеки" in str.lower(message.text)) or (
        "скока до физеки" in str.lower(message.text)):
        now = datetime.datetime.now()
        dphys = phys - now
        bot.send_message(message.chat.id, "До Физона осталось {} дней, {} часов {} минут {} секунд.".format(dphys.days,
                                                                                                            dphys.seconds // 3600,
...
@bot.message_handler(content_types=['new_chat_member' != und])
